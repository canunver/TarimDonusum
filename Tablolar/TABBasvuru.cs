...
namespace TarimDonusum.Tablolar
{
    public class TABBasvuru : TABTablo
    {
        ...
        public async Task OrtakCevreselTaahhutKaydetAsync(int basvuruId, bool onay)
        {
            const string sql = @"UPDATE dbo.Basvuru
                SET TaahhutBeyanlarJson=JSON_MODIFY(CASE WHEN ISJSON(TaahhutBeyanlarJson)=1 THEN TaahhutBeyanlarJson ELSE '{}' END, '$.cevreselSosyalTaahhutOnayi', @Onay)
                WHERE Id=@Id;
                UPDATE dbo.BasvuruUygulamaAdresleri
                SET CevreselSosyalJson=JSON_MODIFY(CevreselSosyalJson, '$.declaration', @Onay)
                WHERE BasvuruId=@Id AND ISJSON(CevreselSosyalJson)=1;";
            await using SqlCommand command = KomutOlustur(sql);
            command.Parameters.AddWithValue("@Id", basvuruId);
            command.Parameters.Add("@Onay", System.Data.SqlDbType.Bit).Value = onay;
            await command.ExecuteNonQueryAsync();
        }
        ...
        public async Task<bool> IncelemeyeGonderAsync(int basvuruId)
        {
            const string sql = @"
                ...
                IF @IlKodu NOT BETWEEN 0 AND 99
                    THROW 50001, N'İl kodu başvuru numarası için 0-99 aralığında olmalıdır.', 1;
                IF TRY_CONVERT(INT, @DegerZinciriKodu) IS NULL OR TRY_CONVERT(INT, @DegerZinciriKodu) NOT BETWEEN 0 AND 99
                    THROW 50002, N'Değer zinciri kodu başvuru numarası için sayısal ve 0-99 aralığında olmalıdır.', 1;
                ...
                SELECT @SiraNo = ISNULL(MAX(TRY_CONVERT(INT, SUBSTRING(BasvuruNo, LEN(@OnEk) + 1, 4))), 0) + 1
                FROM dbo.BasvuruAna WITH (UPDLOCK, HOLDLOCK)
                WHERE DonemId = @DonemId AND BasvuruNo LIKE @OnEk + N'%';

                IF @SiraNo > 9999
                    THROW 50003, N'Dönem, il ve değer zinciri için başvuru numarası sınırına ulaşıldı.', 1;

                UPDATE BA
                SET BA.Durum = @YeniDurum,
                    BA.BasvuruNo = COALESCE(BA.BasvuruNo,
                        CONCAT(@OnEk, RIGHT(N'0000' + CONVERT(NVARCHAR(4), @SiraNo), 4), N'-',
                               RIGHT(N'00' + CONVERT(NVARCHAR(2), MONTH(GETDATE())), 2), N'-',
                               RIGHT(N'0000' + CONVERT(NVARCHAR(4), YEAR(GETDATE())), 4)))
                FROM dbo.BasvuruAna BA
                INNER JOIN dbo.Basvuru B ON B.BasvuruAnaId = BA.Id
                WHERE B.Id = @BasvuruId
        ...
        public async Task<int> YeniRevizyonOlusturAsync(int kaynakBasvuruId, enumBasvuruKayitTuru kayitTuru = enumBasvuruKayitTuru.OnBasvuru)
        {
            const string sql = @"
                ...
                USING (SELECT M.Id,E.YeniId UygulamaAdresiId,M.DegerZinciriAsamaTuru,M.SiraNo,M.Ad,M.Birim,M.Miktar,M.Aciklama,M.Marka,M.Model,M.KapasiteOzellikleri,M.YerlesimPlaniSiraNo,M.KullanimAmaci,M.Durum,M.KapasiteSecimGerekcesi FROM dbo.BasvuruMakine M LEFT JOIN @AdresEsleme E ON E.EskiId=M.UygulamaAdresiId WHERE M.BasvuruId=@KaynakBasvuruId) AS kaynak
                ON 1=0
                WHEN NOT MATCHED THEN INSERT(BasvuruId,UygulamaAdresiId,DegerZinciriAsamaTuru,SiraNo,Ad,Birim,Miktar,Aciklama,Marka,Model,KapasiteOzellikleri,YerlesimPlaniSiraNo,KullanimAmaci,Durum,KapasiteSecimGerekcesi)
                VALUES(@YeniBasvuruId,kaynak.UygulamaAdresiId,kaynak.DegerZinciriAsamaTuru,kaynak.SiraNo,kaynak.Ad,kaynak.Birim,kaynak.Miktar,kaynak.Aciklama,kaynak.Marka,kaynak.Model,kaynak.KapasiteOzellikleri,kaynak.YerlesimPlaniSiraNo,kaynak.KullanimAmaci,kaynak.Durum,kaynak.KapasiteSecimGerekcesi)
        ...
        private static Basvuru Oku(SqlDataReader reader)
        {
            ...
            basvuru.basvuruFirma.basvuruNo = NullOkuString(reader, kol++);
        ...
        private async Task<int> UygulamaAdresiEkleAsync(BasvuruUygulamaAdresi adres)
        {
            ...
            adres.id = id;
            await UygulamaAdresiKonumlariniKaydetAsync(adres);
        ...
        private static void UygulamaAdresiParametreleriEkle(SqlCommand command, BasvuruUygulamaAdresi adres)
        {
            ...
            command.Parameters.AddWithValue("@YatirimFaaliyetleri", DbNull(adres.yatirimFaaliyetleri));
            command.Parameters.AddWithValue("@YatirimGirdileri", DbNull(adres.yatirimGirdileri));
            command.Parameters.AddWithValue("@YatirimCiktilari", DbNull(adres.yatirimCiktilari));
        }

        public async Task BasvuruMakineAdresiGuncelleAsync(BasvuruMakine makine)
        {
            const string sql = @"UPDATE dbo.BasvuruMakine
                SET UygulamaAdresiId=@AdresId
                WHERE Id=@Id AND BasvuruId=@BasvuruId
                  AND EXISTS(SELECT 1 FROM dbo.BasvuruUygulamaAdresleri WHERE Id=@AdresId AND BasvuruId=@BasvuruId);";
            await using SqlCommand command = KomutOlustur(sql);
            command.Parameters.AddWithValue("@AdresId", makine.uygulamaAdresiId!.Value);
            command.Parameters.AddWithValue("@Id", makine.id);
            command.Parameters.AddWithValue("@BasvuruId", makine.basvuruId);
            if (await command.ExecuteNonQueryAsync() == 0)
                throw new InvalidOperationException("Makine için seçilen yatırım adresi bulunamadı.");
        }

        public async Task OnBasvuruYatirimTemelBilgileriKaydetAsync(BasvuruYatirim yatirim)
        {
            const string sql = "UPDATE dbo.Basvuru SET YatirimAdi=@YatirimAdi, YatiriminAmaci=@YatiriminAmaci WHERE Id=@Id;";
            await using SqlCommand command = KomutOlustur(sql);
            command.Parameters.AddWithValue("@YatirimAdi", DbNull(yatirim.yatirimAdi));
            command.Parameters.AddWithValue("@YatiriminAmaci", DbNull(yatirim.yatiriminAmaci));
            command.Parameters.AddWithValue("@Id", yatirim.basvuruId);
        ...
        public async Task CevreselSosyalAdresiKaydetAsync(BasvuruCevreselSosyal cevreselSosyal)
        {
            const string sql = @"
                UPDATE dbo.BasvuruUygulamaAdresleri
                SET CevreselSosyalJson = @CevreselSosyalJson
                WHERE Id = @AdresId AND BasvuruId = @BasvuruId;";
            await using SqlCommand command = KomutOlustur(sql);
            command.Parameters.AddWithValue("@CevreselSosyalJson", DbNull(cevreselSosyal.cevreselSosyalJson));
            command.Parameters.AddWithValue("@AdresId", cevreselSosyal.uygulamaAdresiId);
            command.Parameters.AddWithValue("@BasvuruId", cevreselSosyal.basvuruId);
            if (await command.ExecuteNonQueryAsync() != 1)
                throw new InvalidOperationException("Çevresel-sosyal anket adresi bulunamadı.");
        }
        ...
        private async Task UygulamaAdresiKonumlariniKaydetAsync(BasvuruUygulamaAdresi adres)
        {
            await using (SqlCommand sil = KomutOlustur("DELETE FROM dbo.BasvuruUygulamaAdresiKonum WHERE AdresId=@AdresId;"))
            {
                sil.Parameters.AddWithValue("@AdresId", adres.id);
                await sil.ExecuteNonQueryAsync();
            }
        ...
        private async Task UygulamaAdresiTurleriniKaydetAsync(BasvuruUygulamaAdresi adres)
        {
            const string silSql = @"DELETE FROM dbo.BasvuruUygulamaAdresiYatirimTuru WHERE AdresId=@AdresId;
                                    DELETE FROM dbo.BasvuruUygulamaAdresiHarcamaTuru WHERE AdresId=@AdresId;";
            await using (SqlCommand sil = KomutOlustur(silSql))
            {
                sil.Parameters.AddWithValue("@AdresId", adres.id);
                await sil.ExecuteNonQueryAsync();
            }
            foreach (int tur in (adres.yatirimTurleri ?? []).Where(x => x > 0).Distinct())
            {
                await using SqlCommand ekle = KomutOlustur("INSERT INTO dbo.BasvuruUygulamaAdresiYatirimTuru(AdresId,YatirimTuru) VALUES(@AdresId,@Tur);");
                ekle.Parameters.AddWithValue("@AdresId", adres.id); ekle.Parameters.AddWithValue("@Tur", tur);
                await ekle.ExecuteNonQueryAsync();
            }
            foreach (int tur in (adres.harcamaTurleri ?? []).Where(x => x > 0).Distinct())
            {
                await using SqlCommand ekle = KomutOlustur("INSERT INTO dbo.BasvuruUygulamaAdresiHarcamaTuru(AdresId,HarcamaTuru) VALUES(@AdresId,@Tur);");
                ekle.Parameters.AddWithValue("@AdresId", adres.id); ekle.Parameters.AddWithValue("@Tur", tur);
                await ekle.ExecuteNonQueryAsync();
            }
        }
        ...
        private async Task DetaylariYukleAsync(Basvuru basvuru)
        {
            string sql = AdresSorguOl() + @" WHERE bua.BasvuruId = @BasvuruId ORDER BY bua.SiraNo;
                ...
                SELECT Id,BasvuruId,SiraNo,Ad,Birim,Miktar,Aciklama,Marka,Model,KapasiteOzellikleri,YerlesimPlaniSiraNo,KullanimAmaci,Durum,KapasiteSecimGerekcesi,UzmanParaBirimi,UzmanKur,UzmanMinimumFiyat,
                    UzmanMaksimumFiyat,UzmanSecilenTeklifId,UzmanOnerilenFiyatTl,UzmanKontrolSonucu,UzmanAciklama,UygulamaAdresiId,DegerZinciriAsamaTuru
            ...
            while (await reader.ReadAsync())
            {
                ...
                dza.asamaTuru = reader.IsDBNull(kolNo) ? null : (enumDegerZinciriAsamaTuru)reader.GetInt32(kolNo); kolNo++;
            ...
            while (await reader.ReadAsync())
            {
                ...
                makineler[^1].uygulamaAdresiId=NullOkuInt(reader,k++);
                makineler[^1].degerZinciriAsamaTuru=reader.IsDBNull(k)?null:(enumDegerZinciriAsamaTuru)reader.GetInt32(k);k++;
            }
            await reader.NextResultAsync();
            while (await reader.ReadAsync())
            {
            ...
            basvuru.yatirimOzeti.yatirimOzetiJson = TeknikProjeGiderleri.Birlestir(basvuru.yatirimOzeti.yatirimOzetiJson, yatirimOnBilgileri);
            basvuru.yatirimOzeti.urunler = yatirimOnBilgileri
                .Where(x => x.tur is enumYatirimOnBilgiTuru.MevcutUrun or enumYatirimOnBilgiTuru.UretilecekUrun)
                .ToList();
            basvuru.Makineler = makineler;
            foreach (BasvuruMakine makine in basvuru.Makineler)
            {
                BasvuruUygulamaAdresi? adres = adresler.FirstOrDefault(x => x.id == makine.uygulamaAdresiId);
                makine.uygulamaAdresiAciklama = adres == null ? "" : $"{adres.siraNo}. {adres.ilAdi} / {adres.ilceAdi} - {adres.tamAdres}";
            }
            basvuru.UrunSurecleri = urunSurecleri;
            basvuru.Binalar = binalar;
            foreach (BasvuruBina bina in basvuru.Binalar)
            {
                BasvuruUygulamaAdresi? adres = adresler.FirstOrDefault(x => x.id == bina.uygulamaAdresiId);
                bina.uygulamaAdresiAciklama = adres == null ? "" : $"{adres.siraNo}. {adres.ilAdi} / {adres.ilceAdi} - {adres.tamAdres}";
            }
            basvuru.TedarikciEntegrasyonlari = tedarikler;
            basvuru.IzlemeGostergeleri = izleme;
            basvuru.TeknikProjeUrunleriniDoldur();
        }
        ...
        public async Task BasvuruYatirimOnBilgileriKaydetAsync(int basvuruId, List<BasvuruYatirimOnBilgi> kayitlar)
        {
            ...
            const string sql=@"INSERT dbo.BasvuruYatirimOnBilgi(BasvuruId,Tur,SiraNo,Ad,Miktar,Birim,TekPanelGucu,TekPanelGucuBirim,ToplamGuc,ToplamGucBirim,MevcutKapasite,MevcutUretimMiktari,BirinciYilKapasite,BirinciYilUretimMiktari,SatisMiktari,BirimSatisFiyati,MevcutSatisMiktari,MevcutBirimSatisFiyati,UygulamaAdresiId,DegerZinciriAsamaTuru,GiderTuru,SabitOran,BirimFiyat)
                OUTPUT INSERTED.Id VALUES(@BasvuruId,@Tur,@SiraNo,@Ad,@Miktar,@Birim,@TekPanelGucu,@TekPanelGucuBirim,@ToplamGuc,@ToplamGucBirim,@MevcutKapasite,@MevcutUretimMiktari,@BirinciYilKapasite,@BirinciYilUretimMiktari,@SatisMiktari,@BirimSatisFiyati,@MevcutSatisMiktari,@MevcutBirimSatisFiyati,@UygulamaAdresiId,@DegerZinciriAsamaTuru,@GiderTuru,@SabitOran,@BirimFiyat);";
        ...
        {
            if(m.id>0){await using SqlCommand c=KomutOlustur(@"UPDATE dbo.BasvuruMakine SET UygulamaAdresiId=@UygulamaAdresiId,DegerZinciriAsamaTuru=@DegerZinciriAsamaTuru,SiraNo=@SiraNo,Ad=@Ad,Birim=@Birim,Miktar=@Miktar,Aciklama=@Aciklama,Marka=@Marka,Model=@Model,KapasiteOzellikleri=@KapasiteOzellikleri,YerlesimPlaniSiraNo=@YerlesimPlaniSiraNo,KullanimAmaci=@KullanimAmaci,Durum=@Durum,KapasiteSecimGerekcesi=@KapasiteSecimGerekcesi WHERE Id=@Id AND BasvuruId=@BasvuruId AND EXISTS(SELECT 1 FROM dbo.BasvuruUygulamaAdresleri WHERE Id=@UygulamaAdresiId AND BasvuruId=@BasvuruId);");MakineParametreleri(c,m);c.Parameters.AddWithValue("@UygulamaAdresiId",DbNull(m.uygulamaAdresiId));c.Parameters.AddWithValue("@Id",m.id);if(await c.ExecuteNonQueryAsync()==0)throw new InvalidOperationException("Makine kaydı veya seçilen adres bulunamadı.");}
            else{await using SqlCommand c=KomutOlustur(@"INSERT dbo.BasvuruMakine(BasvuruId,UygulamaAdresiId,DegerZinciriAsamaTuru,SiraNo,Ad,Birim,Miktar,Aciklama,Marka,Model,KapasiteOzellikleri,YerlesimPlaniSiraNo,KullanimAmaci,Durum,KapasiteSecimGerekcesi) OUTPUT INSERTED.Id SELECT @BasvuruId,@UygulamaAdresiId,@DegerZinciriAsamaTuru,@SiraNo,@Ad,@Birim,@Miktar,@Aciklama,@Marka,@Model,@KapasiteOzellikleri,@YerlesimPlaniSiraNo,@KullanimAmaci,@Durum,@KapasiteSecimGerekcesi WHERE EXISTS(SELECT 1 FROM dbo.BasvuruUygulamaAdresleri WHERE Id=@UygulamaAdresiId AND BasvuruId=@BasvuruId);");MakineParametreleri(c,m);c.Parameters.AddWithValue("@UygulamaAdresiId",DbNull(m.uygulamaAdresiId));object? yeniId=await c.ExecuteScalarAsync();if(yeniId==null)throw new InvalidOperationException("Seçilen yatırım adresi bulunamadı.");m.id=Convert.ToInt32(yeniId);}
        ...
        private static BasvuruUygulamaAdresi UygulamaAdresiOku(SqlDataReader reader, IStringLocalizer<SharedResource>? l)
        {
            BasvuruUygulamaAdresi bu = new BasvuruUygulamaAdresi
            {
                ...
                konumlar = JsonSerializer.Deserialize<List<BasvuruUygulamaAdresiKonum>>(reader.GetString(36)) ?? new()
            };
            if (l != null)
            {
        ...
        private static List<int> VirgulluIntListeOku(string? deger)
        {
            return (deger ?? "").Split(',', StringSplitOptions.RemoveEmptyEntries)
