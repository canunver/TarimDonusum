...
SELECT mb.Id,mb.BinaId,mb.SiraNo,mb.Ad,mb.Kategori FROM dbo.BasvuruMetrajBolum mb INNER JOIN dbo.BasvuruBina b ON b.Id=mb.BinaId WHERE b.BasvuruId=@BasvuruId ORDER BY mb.BinaId,mb.SiraNo,mb.Id;
...
SELECT d.Id,d.MetrajPozId,d.SiraNo,d.Aciklama,d.BenzerSayisi,d.Adet,d.Boy,d.En,d.Yukseklik,d.UrunCinsi,d.BirimAgirlik FROM dbo.BasvuruMetrajDetay d INNER JOIN dbo.BasvuruMetrajPoz mp ON mp.Id=d.MetrajPozId INNER JOIN dbo.BasvuruMetrajBolum mb ON mb.Id=mp.BolumId INNER JOIN dbo.BasvuruBina b ON b.Id=mb.BinaId WHERE b.BasvuruId=@BasvuruId ORDER BY d.MetrajPozId,d.SiraNo,d.Id;
SELECT p.Id,@DonemId,p.Id,f.BirimFiyat,p.PozNo,p.Ad,p.Birim,p.HesaplamaTuru FROM dbo.Poz p LEFT JOIN dbo.PozDonemFiyat f ON f.PozId=p.Id AND f.DonemId=@DonemId WHERE p.Aktif=1 ORDER BY p.PozNo;";
            ...
            await r.NextResultAsync();while(await r.ReadAsync()){BasvuruMetrajBolum x=new(){id=r.GetInt32(0),basvuruId=basvuruId,binaId=r.GetInt32(1),siraNo=r.GetInt32(2),ad=r.GetString(3),kategori=r.IsDBNull(4)?0:(enumMetrajKategori)r.GetInt32(4)};if(Enum.IsDefined(x.kategori))x.ad=MetrajKategoriTanimlari.Ad(x.kategori);v.binalar.FirstOrDefault(b=>b.id==x.binaId)?.bolumler.Add(x);}
            ...
            await r.NextResultAsync();while(await r.ReadAsync()){BasvuruMetrajDetay d=new(){id=r.GetInt32(0),siraNo=r.GetInt32(2),aciklama=r.IsDBNull(3)?"":r.GetString(3),benzerSayisi=r.GetDecimal(4),adet=NullOkuDecimal(r,5),boy=NullOkuDecimal(r,6),en=NullOkuDecimal(r,7),yukseklik=NullOkuDecimal(r,8),urunCinsi=r.IsDBNull(9)?"":r.GetString(9),birimAgirlik=NullOkuDecimal(r,10)};pozlar.FirstOrDefault(x=>x.id==r.GetInt32(1))?.detaylar.Add(d);}
            await r.NextResultAsync();while(await r.ReadAsync())v.pozlar.Add(new(){id=r.GetInt32(0),donemId=r.GetInt32(1),pozId=r.GetInt32(2),birimFiyat=NullOkuDecimal(r,3),pozNo=r.GetString(4),pozAdi=r.GetString(5),birim=r.GetString(6),hesaplamaTuru=r.GetInt32(7)});
            return v;
        }

        public async Task BolumKaydetAsync(BasvuruMetrajBolum x)
        {
            string sql=x.id>0?"UPDATE mb SET SiraNo=@SiraNo,Ad=@Ad,Kategori=@Kategori FROM dbo.BasvuruMetrajBolum mb INNER JOIN dbo.BasvuruBina b ON b.Id=mb.BinaId WHERE mb.Id=@Id AND b.BasvuruId=@BasvuruId;":"INSERT dbo.BasvuruMetrajBolum(BinaId,SiraNo,Ad,Kategori) SELECT @BinaId,@SiraNo,@Ad,@Kategori FROM dbo.BasvuruBina WHERE Id=@BinaId AND BasvuruId=@BasvuruId AND MevcutYeni=N'Yeni';SELECT CAST(SCOPE_IDENTITY() AS INT);";
            await using SqlCommand c=KomutOlustur(sql);c.Parameters.AddWithValue("@Id",x.id);c.Parameters.AddWithValue("@BasvuruId",x.basvuruId);c.Parameters.AddWithValue("@BinaId",x.binaId);c.Parameters.AddWithValue("@SiraNo",x.siraNo);c.Parameters.AddWithValue("@Ad",x.ad);c.Parameters.AddWithValue("@Kategori",(int)x.kategori);if(x.id>0){if(await c.ExecuteNonQueryAsync()==0)throw new InvalidOperationException("Yapım bölümü bulunamadı.");}else x.id=Convert.ToInt32(await c.ExecuteScalarAsync()??throw new InvalidOperationException("Yeni bina bulunamadı."));
        }
        ...
        public async Task PozKaydetAsync(BasvuruMetrajPoz x)
        {
            ...
            foreach(BasvuruMetrajDetay d in x.detaylar){await using SqlCommand c=KomutOlustur("INSERT dbo.BasvuruMetrajDetay(MetrajPozId,SiraNo,Aciklama,UrunCinsi,BirimAgirlik,BenzerSayisi,Adet,Boy,En,Yukseklik) VALUES(@PozId,@SiraNo,@Aciklama,@UrunCinsi,@BirimAgirlik,@BenzerSayisi,@Adet,@Boy,@En,@Yukseklik);");c.Parameters.AddWithValue("@PozId",x.id);c.Parameters.AddWithValue("@SiraNo",d.siraNo);c.Parameters.AddWithValue("@Aciklama",string.IsNullOrWhiteSpace(d.aciklama)?DBNull.Value:d.aciklama);c.Parameters.AddWithValue("@UrunCinsi",string.IsNullOrWhiteSpace(d.urunCinsi)?DBNull.Value:d.urunCinsi);c.Parameters.AddWithValue("@BirimAgirlik",(object?)d.birimAgirlik??DBNull.Value);c.Parameters.AddWithValue("@BenzerSayisi",d.benzerSayisi);c.Parameters.AddWithValue("@Adet",(object?)d.adet??DBNull.Value);c.Parameters.AddWithValue("@Boy",(object?)d.boy??DBNull.Value);c.Parameters.AddWithValue("@En",(object?)d.en??DBNull.Value);c.Parameters.AddWithValue("@Yukseklik",(object?)d.yukseklik??DBNull.Value);await c.ExecuteNonQueryAsync();}
        }
