...
public sealed class RPROB_YatirimOzeti(string uygulamaRootPath) : RPROBTemel(uygulamaRootPath)
{
    ...
    private static List<UrunSatiri> UrunleriOku(string? json)
    {
        ...
        {
            string ad = satir.TryGetProperty("name", out JsonElement a) ? a.GetString() ?? "" : "";
            string birim = satir.TryGetProperty("unit", out JsonElement b) ? b.GetString() ?? "" : "";
            ...
            urunler.Add(new UrunSatiri(ad, birim, veriler));
        }
        return urunler;
    }
