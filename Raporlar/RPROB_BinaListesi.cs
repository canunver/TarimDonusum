...
public sealed class RPROB_BinaListesi(string uygulamaRootPath) : RPROBTemel(uygulamaRootPath)
{
    ...
    protected override void Doldur(Tablo tablo, Basvuru basvuru)
    {
        ...
        tablo.HucreAdAdresCoz("FormatSatir",ref fs,ref fr,ref fc,ref r2,ref c2);tablo.HucreAdAdresCoz("BaslaSatir",ref hs,ref hr,ref hc,ref hr2,ref hc2);
        ...
        for(int i=0;i<satirlar.Count;i++)
        {
            ...
            for(int c=0;c<d.Length;c++)tablo.HucreDegerYaz(hedef,hc+c,d[c]);tablo.SatirYukseklikAyarla(hedef,hedef,-1,0,min);
        }
        tablo.AktifSheetDegistir(fs);tablo.SatirSil(fr,fr);
    }
    private static string X(string deger,string beklenen)=>deger==beklenen?"X":"";
    private static List<Satir> Oku(string? json){List<Satir> sonuc=[];if(string.IsNullOrWhiteSpace(json))return sonuc;using System.Text.Json.JsonDocument b=System.Text.Json.JsonDocument.Parse(json);if(!b.RootElement.TryGetProperty("buildingRows",out var d)||d.ValueKind!=System.Text.Json.JsonValueKind.Array)return sonuc;foreach(var x in d.EnumerateArray()){Satir s=new(RaporJson.DegerMetni(x,"name"),RaporJson.DegerMetni(x,"assetStatus"),RaporJson.DegerMetni(x,"investmentType"),RaporJson.DegerMetni(x,"supportRequested"));if(!string.IsNullOrWhiteSpace(s.Ad))sonuc.Add(s);}return sonuc;}
