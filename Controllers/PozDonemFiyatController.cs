...
{
    [OturumKontrol]
    public class PozDonemFiyatController : BMYController
    {
        private readonly BasvuruIsKurallari _basvuru;
        private readonly TanimIsKurallari _tanim;
        public PozDonemFiyatController(ILoggerFactory loggerFactory, IStringLocalizer<SharedResource> localizer,
        ...
        [HttpGet] public async Task<IActionResult> SayfaVerisi(int donemId, bool listele = false) => Json(await _tanim.PozDonemFiyatSayfaVerisiAsync(donemId, await KullaniciAsync(), listele));
        [HttpPost] public async Task<IActionResult> Kaydet([FromBody] PozDonemFiyatKayit model) => Json(await _tanim.PozDonemFiyatlariKaydetAsync(model, await KullaniciAsync()));

        [HttpPost]
        [RequestSizeLimit(10 * 1024 * 1024)]
        public async Task<IActionResult> ExcelYukle(int donemId, IFormFile? dosya)
        {
            ...
            return Json(await _tanim.PozDonemFiyatExcelYukleAsync(donemId, stream, await KullaniciAsync()));
        }
    }
}
